...
void scheduler_code()
{
	int i;
  	...
  	{
    	//pthread_create(&pool[i],NULL,message_queue,(void *)NULL);
  	}
}
...
void mysighup()
{
	printf("SIGHUP signal caught! User's terminal is disconnected!\n");
	return;
}
...
int main(int argc, char ** argv)
{
	...
	//threads create
	//pthread_create(&scheduler_thread,NULL,scheduler_code,(void *)NULL);

	//shared memory
	if((shmid = shmget(IPC_PRIVATE, sizeof(shared_memory), IPC_CREAT | 0766)) < 0) 
	{
